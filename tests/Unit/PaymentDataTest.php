...
describe('PaymentData', function (): void {
    it('deserializes from a full API response', function (): void {
        ...
        expect($data->id)->toBe('pay_123')
            ...
            ->and($data->discount)->toBe(['value' => 5.0, 'type' => 'PERCENTAGE', 'dueDateLimitDays' => 0]);
    });
...
describe('CreatePaymentData', function (): void {
    ...
    it('includes nested arrays when provided', function (): void {
        $dto = new CreatePaymentData(
            customer: 'cus_abc',
            billingType: 'BOLETO',
            value: 200.0,
            dueDate: '2026-04-01',
            discount: ['value' => 10.0, 'type' => 'FIXED', 'dueDateLimitDays' => 5],
            fine: ['value' => 2.0],
        );

        $payload = $dto->toArray();

        expect($payload)->toHaveKey('discount', ['value' => 10.0, 'type' => 'FIXED', 'dueDateLimitDays' => 5])
            ->and($payload)->toHaveKey('fine', ['value' => 2.0]);
    });

    it('throws when customer is empty', function (): void {
        new CreatePaymentData(customer: '', billingType: 'PIX', value: 100.0, dueDate: '2026-04-01');
    })->throws(InvalidArgumentException::class, 'Payment customer cannot be empty.');

    it('throws when billingType is empty', function (): void {
        new CreatePaymentData(customer: 'cus_abc', billingType: '', value: 100.0, dueDate: '2026-04-01');
    })->throws(InvalidArgumentException::class, 'Payment billingType cannot be empty.');

    it('throws when dueDate is empty', function (): void {
        new CreatePaymentData(customer: 'cus_abc', billingType: 'PIX', value: 100.0, dueDate: '');
    })->throws(InvalidArgumentException::class, 'Payment dueDate cannot be empty.');

    it('throws when value is zero', function (): void {
        new CreatePaymentData(customer: 'cus_abc', billingType: 'PIX', value: 0.0, dueDate: '2026-04-01');
    ...
    it('throws when value is negative', function (): void {
        new CreatePaymentData(customer: 'cus_abc', billingType: 'PIX', value: -5.0, dueDate: '2026-04-01');
    })->throws(InvalidArgumentException::class, 'Payment value must be greater than zero.');
});
...
describe('PaymentListFilters', function (): void {
    ...
    it('includes optional string filters when set', function (): void {
        $filters = new PaymentListFilters(customer: 'cus_abc', billingType: 'PIX', status: 'PENDING');
