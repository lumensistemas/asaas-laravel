...
describe('CustomerService::list()', function (): void {
    it('returns a CustomerListResult with data', function (): void {
...
describe('CustomerService::find()', function (): void {
    it('returns a CustomerData for a valid ID', function (): void {
        ...
        Http::assertSent(
            fn ($request): bool
            => str_ends_with((string) $request->url(), '/v3/customers/cus_000005401844')
            && $request->method() === 'GET'
        );
    });
});

describe('CustomerService::create()', function (): void {
    it('POSTs and returns the created customer', function (): void {
        Http::fake(['*' => Http::response(customerPayload())]);
        ...
        Http::assertSent(
            fn ($request): bool
            => str_ends_with((string) $request->url(), '/v3/customers')
            && $request->method() === 'POST'
    ...
    it('throws AsaasApiException when the API rejects the payload', function (): void {
        Http::fake(['*' => Http::response(
...
describe('CustomerService::update()', function (): void {
    it('PUTs and returns the updated customer', function (): void {
...
describe('CustomerService::delete()', function (): void {
    it('DELETEs the customer and returns true', function (): void {
...
describe('CustomerService::restore()', function (): void {
    it('POSTs to the restore endpoint and returns the customer', function (): void {
        ...
        Http::assertSent(
            fn ($request): bool
            => str_ends_with((string) $request->url(), '/v3/customers/cus_000005401844/restore')
            && $request->method() === 'POST'
        );
    });
});

describe('Multi-tenant key injection', function (): void {
    it('uses the tenant key and does not mutate the default instance', function (): void {
        Http::fake(['*' => Http::response(customerPayload())]);

        Asaas::withApiKey('tenant_abc')->customers()->find('cus_000005401844');

        Http::assertSent(fn ($request): bool => $request->header('access_token')[0] === 'tenant_abc');

        // The facade's default instance still uses the configured key
        Asaas::customers()->find('cus_000005401844');

        Http::assertSent(fn ($request): bool => $request->header('access_token')[0] === 'test_api_key');
    });
});
